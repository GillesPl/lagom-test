...
/**
  ...
  */
class UserServiceImpl(persistentEntityRegistry: PersistentEntityRegistry, system: ActorSystem)(implicit ec: ExecutionContext, mat: Materializer) extends UserService {
  ...
  override def getUsers = ServiceCall { _ =>
    ...
    currentIdsQuery.currentPersistenceIds()
      ...
      .mapAsync(4) { id =>
        ...
        persistentEntityRegistry.refFor[UserEntity](entityId)
          .ask(GetUser)
  ...
  private def refFor(userId: UUID) = persistentEntityRegistry.refFor[UserEntity](userId.toString)
}
